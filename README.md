...
**选项 A: 基础功能测试 (Basic Seed)**
生成基础的订单簿深度和简单的持仓数据，适合测试基础交易功能。
```bash
./scripts/seed.sh
```
*场景描述：Alice 和 Bob 在 2000 U 附近挂单，并进行少量的撮合交易，验证系统流动性。*

**选项 B: 排行榜博弈演示 (Leaderboard Seed)**
模拟一场激烈的多空博弈，生成高额盈亏数据，点亮排行榜。
```bash
./scripts/seed_leaderboard.sh
```
*博弈剧本：*
1.  **开局**: Alice (多) 和 Bob (空) 在 2000 U 建仓，平稳交易，互有胜负。
2.  **高潮**: Carol (巨鲸) 进场，在 3000 U 高位重仓做空 5 ETH；Alice 选择做多接盘。
3.  **决战**: 市场价格崩盘至 2500 U。
4.  **结局**: Carol 大赚 2500 U 登顶榜首，Alice 巨亏垫底。此过程完整展示了保证金扣除、盈亏结算和排行榜更新流程。
